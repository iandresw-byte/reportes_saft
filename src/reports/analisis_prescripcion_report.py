...
class MoravsBIAldeaAnioReport:
    ...
    def generar_pdf(self, ruta_salida="mora_vs_ingresos_gral.pdf"):
        ...
        filas.append(['', '', f"{total_prescrito:,.2f}", f"{total_recuperable:,.2f}",
                     '', '', f"{total_saldo:,.2f}", total_percent_recuperable])
    ...
    def generar_excel(self, ruta_salida="analisi_prescrito.xlsx"):
        wb = Workbook()
        ws = wb.active
        ...
        data = Reference(ws, min_col=11, min_row=fila,
                         max_row=fila_fin)
