...
from src.ui.components.ui_style_table import columa_style, fila_style, fila_style_moneda, fila_style_moneda_total, table_estyle_aviso, table_style_UMA


def tabla_aviso_mora_media_carta(datos) -> Table:
    estilo_encabezado = columa_style()
    estilo_fila = fila_style()
    estilo_tabla = table_estyle_aviso()
    ...
    tabla_der = Table(filas_der, colWidths=[50, 170, 55])
    tabla_der.setStyle(estilo_tabla)
    tabla_izq = Table(filas_izq, colWidths=[50, 170, 55])
