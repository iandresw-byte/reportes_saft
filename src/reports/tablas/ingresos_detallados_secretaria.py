...
from src.reports.utils.manejador_data_depto_diario_frame import sumar_ingresos_depto_diarios_secretaria
from src.reports.utils.manejador_data_frame import sumar_ingresos_departo_secretaria
from src.reports.utils.manejador_data_depto_mensual import sumar_ingresos_depto_mensual_secretaria
...
def tabla_ingreso_detallado_scretaria(datos, ) -> Table:
    ...
    datos = sumar_ingresos_departo_secretaria(datos)
...
def tabla_ingreso_diario_scretaria(datos, ) -> Table:
    ...
    encabezados = []
    encabezados.append(Paragraph("No.", estilo_encabezado),)
    datos = sumar_ingresos_depto_diarios_secretaria(datos)
    for col in datos.columns:
        encabezados.append(Paragraph(col, estilo_encabezado),)
    filas = [encabezados]
    for index, item in datos.iterrows():
...
def tabla_ingreso_mensual_scretaria(datos, ) -> Table:
    estilo_encabezado = columa_style()
    estilo_fila = fila_style()
    estilo_tabla = table_style_UMA()
    estilo_fila_moneda = fila_style_moneda()
    ...
    encabezados = []
    encabezados.append(Paragraph("No.", estilo_encabezado),)
    datos = sumar_ingresos_depto_mensual_secretaria(datos)
    for col in datos.columns:
        encabezados.append(Paragraph(col, estilo_encabezado),)
    filas = [encabezados]
    for index, item in datos.iterrows():
...
def tabla_ingreso_diario_admin_excel_secretaria(excel, datos, ws):
    for index, item in datos.iterrows():
        ixd = index+1
        excel.fila_num += 1
        ws.append([
