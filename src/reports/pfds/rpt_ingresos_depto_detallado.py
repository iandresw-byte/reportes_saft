...
from src.reports.tablas.ingresos_detallados_bomberos import tabla_ingreso_detallado_bomberos
from src.reports.tablas.ingresos_detallados_admin_triburaria_os import tabla_ingreso_detallado_admin_tributaria_os
from src.reports.tablas.ingresos_detallados_admin_triburaria_ip import tabla_ingreso_detallado_admin_tributaria_ip
from src.reports.tablas.ingresos_detallados_admin_triburaria_bi import tabla_ingreso_detallado_admin_tributaria_bi
from src.reports.tablas.ingresos_detallados_admin_tributaria_ics import tabla_ingreso_detallado_admin_tributaria_ics
from src.reports.tablas.ingresos_detallados_catastro import tabla_ingreso_detallado_catastro
from src.reports.tablas.ingresos_detallados_secretaria import tabla_ingreso_detallado_scretaria
...
from src.reports.utils.tabla_ingresos_uma import tabla_ingreso_detallado_uma
from src.reports.utils.tabla_titulo_pfd import titulo_horizontal, titulo_paragrap_pdf


class RptIngresosDeptoDetalladoReport:
    ...
    def generar_pdf(self, ruta_salida="rpt_ingresos_depto_detatllado.pdf"):
        ...
        # CATASTRO
        if self.depto == "1":
            tabla = tabla_ingreso_detallado_catastro(self.datos)
        ...
        # ADMINISTRACION TRIBUTARIA
            ...
            if self.tipo_factura == "0":
                tabla = tabla_ingreso_detallado_admin_tributaria_os(self.datos)
                elementos.append(tabla)
                tabla_firma = tabla_frima_pdf_unica(
                    self.administracion["Tributaria"], "Jefe del Departamento de Administracion Tributaria", None)
            elif self.tipo_factura == "1":
                tabla = tabla_ingreso_detallado_admin_tributaria_bi(self.datos)
                elementos.append(tabla)
                tabla_firma = tabla_frima_pdf_unica(
                    self.administracion["Tributaria"], "Jefe del Departamento de Administracion Tributaria", None)
            elif self.tipo_factura == "2,3":
                tabla = tabla_ingreso_detallado_admin_tributaria_ics(
                    self.datos)
                elementos.append(tabla)
                tabla_firma = tabla_frima_pdf_unica(
                    self.administracion["Tributaria"], "Jefe del Departamento de Administracion Tributaria", None)
        ...
        # UNIDAD AMBIENTAL
            ...
            tabla = tabla_ingreso_detallado_uma(self.datos)
            elementos.append(tabla)
            tabla_firma = tabla_frima_pdf_unica(
                self.administracion["Ambiental"], "Jefe de la Unidad Ambiental Municipal", None)
        # DIRECCION DE JUSTICIA MUNICIPAL
        elif self.depto == "4":
            tabla = tabla_ingreso_detallado_justicia(self.datos)
            elementos.append(tabla)
            tabla_firma = tabla_frima_pdf_unica(
                self.administracion["Justicia"], "Jefe de la Unidad de Direcion de Justicias Municipal", None)
        # SECRETARIA
        elif self.depto == "6":
            tabla = tabla_ingreso_detallado_scretaria(self.datos)
            elementos.append(tabla)
            tabla_firma = tabla_frima_pdf_unica(
                self.administracion["Secretaria"], "Jefe del Departamento de Secretaria Municipal", None)
        # PROCAMUT
        elif self.depto == "7":
            tabla = tabla_ingreso_detallado_procamut(self.datos)
            elementos.append(tabla)
            tabla_firma = tabla_frima_pdf_unica(
                self.administracion["Justicia"], "", None)
        # DESARROLLO URBANO
        elif self.depto == "8":
            tabla = tabla_ingreso_detallado_desarrollo_urbano(self.datos)
            elementos.append(tabla)
            tabla_firma = tabla_frima_pdf_unica(
                self.administracion["DesarrolloUrbano"], "Jefe Unidad de Desarrollo Urbano", None)

        elif self.depto == "BOMBEROS":
            tabla = tabla_ingreso_detallado_bomberos(
                self.datos, tipo_cta=self.tipo_cta)
            elementos.append(tabla)
            tabla_firma = tabla_frima_pdf_unica(
