...
from src.ui.components.ui_style_table import table_per_ope, estilos_parrafo, table_per_ope_2
...
class PermisoOperacionReport:
    def __init__(self, datos: Tra_PermOpe, municipio, titulo_reporte, firma_justicia=False, municipio_admin=None):
        self.datos = datos
...
# TABLA QR
        num_permiso = Paragraph(
            f"{self.datos.NoPermiso:04d}",  estilos["NumPermiso"], )
...
# TABLA FIRMA
        if self.administracion['FirmaPO_1'] == 0:
            firma1 = self.administracion['Tributaria']
            firma_label = 'Administracion Tributaria'
        elif self.administracion['FirmaPO_1'] == 1:
            firma1 = self.administracion['Tesorero']
            firma_label = 'Tesoreria Municipal'
        elif self.administracion['FirmaPO_1'] == 2:
            firma1 = self.administracion['Alcalde']
            firma_label = 'Alcalde Municipal'
        else:
            firma_label = "Firma Autirizada"

        if not self.justicia_firma:
            valores_fila = [
                Paragraph("",),
                Paragraph("",),
                Paragraph(firma_label, estilos["Firma"]),
                Paragraph("",),
                Paragraph("",),
            ]
            tabla_firma = Table(
