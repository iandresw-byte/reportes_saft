...
class MoraSPReport:
    def __init__(self, datos_agua, datos_alcantarillado, datos_tren, datos_bombero, datos_solares, datos_parques, datos_lim_cementerio, datos_ase_cementerio, datos_ambiente, datos_contribucion, datos_interes=[], municipio='', titulo_reporte=''):
        ...
        self.datos_Intres = datos_interes
    ...
    def generar_pdf(self, ruta_salida="mora_bi_report.pdf"):
        ...
        if self.datos_Intres:
            subtitulo_intereses = Paragraph(
                f"<b>Intereses y recargos por servicios</b>",
                estilos["Normal"],
            )
            elementos.append(Spacer(1, 12))
            elementos.append(subtitulo_intereses)
            elementos.append(Spacer(1, 24))
            ...
            filas.append(["", "SUB-TOTAL", f"{sub_total:,.2f}"])
            filas.append(["", "TOTAL", f"{total:,.2f}"])
            tabla_intereses = Table(filas, colWidths=[90, 300, 100])
            tabla_intereses.setStyle(stylo_table)
            elementos.append(tabla_intereses)
