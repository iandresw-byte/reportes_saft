...
from src.reports.tablas.ingresos_detallados_admin_tributaria_ab import tabla_ingreso_mensual_admin_tributaria_ab
from src.reports.tablas.ingresos_detallados_justicia_municipal import tabla_ingreso_mensual_justicia
from src.reports.tablas.ingresos_detallados_procamut import tabla_ingreso_mensual_procamut
from src.reports.tablas.ingresosd_detallados_desarrollo_urbano import tabla_ingreso_mensual_desarrollo_urbano
from src.reports.tablas.ingresos_detallados_admin_triburaria_os import tabla_ingreso_mensual_admin_tributaria_os
from src.reports.tablas.ingresos_detallados_admin_triburaria_ip import tabla_ingreso_mensual_admin_tributaria_ip
from src.reports.tablas.ingresos_detallados_admin_triburaria_bi import tabla_ingreso_mensual_admin_tributaria_bi
from src.reports.tablas.ingresos_detallados_admin_tributaria_ics import tabla_ingreso_mensual_admin_tributaria_ics
from src.reports.tablas.ingresos_detallados_catastro import tabla_ingreso_mensual_catastro
from src.reports.tablas.ingresos_detallados_secretaria import tabla_ingreso_mensual_scretaria
...
from src.reports.tablas.ingresos_detallados_uma import tabla_ingreso_mensual_uma
from src.reports.utils.tabla_titulo_pfd import titulo_horizontal, titulo_paragrap_pdf


class RptIngresosDeptoMensualReport:
    ...
    def generar_pdf(self, ruta_salida="rpt_ingresos_depto_detatllado.pdf"):
        ...
        # CATASTRO
        if self.depto == "1":
            tabla = tabla_ingreso_mensual_catastro(self.datos)
        ...
        # ADMINISTRACION TRIBUTARIA
            ...
            if self.tipo_factura == "0":
                tabla = tabla_ingreso_mensual_admin_tributaria_os(self.datos)
                elementos.append(tabla)
                tabla_firma = tabla_frima_pdf_unica(
                    self.administracion["Tributaria"], "Jefe del Departamento de Administracion Tributaria", None)
            elif self.tipo_factura == "1":
                tabla = tabla_ingreso_mensual_admin_tributaria_bi(self.datos)
                elementos.append(tabla)
                tabla_firma = tabla_frima_pdf_unica(
                    self.administracion["Tributaria"], "Jefe del Departamento de Administracion Tributaria", None)
            elif self.tipo_factura == "2,3":
                tabla = tabla_ingreso_mensual_admin_tributaria_ics(
                    self.datos)
                elementos.append(tabla)
                tabla_firma = tabla_frima_pdf_unica(
                    self.administracion["Tributaria"], "Jefe del Departamento de Administracion Tributaria", None)
            elif self.tipo_factura == "4":
                tabla = tabla_ingreso_mensual_admin_tributaria_ip(
                    self.datos)
                elementos.append(tabla)
                tabla_firma = tabla_frima_pdf_unica(
                    self.administracion["Tributaria"], "Jefe del Departamento de Administracion Tributaria", None)
            elif self.tipo_factura == "8":
                tabla = tabla_ingreso_mensual_admin_tributaria_ab(
                    self.datos)
                elementos.append(tabla)
                tabla_firma = tabla_frima_pdf_unica(
                    self.administracion["Tributaria"], "Jefe del Departamento de Administracion Tributaria", None)
