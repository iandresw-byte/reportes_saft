...
from src.reports.tablas.establecimientos_x_actividad import tabla_establecimietos_x_actividad_excel
...
class EstablecimientosExcelReport:
    ...
    def generar_excel(self, ruta_salida="mora_establecimientos_actividad.xlsx"):
        wb = Workbook()
        ws = wb.active
        ws.title = "Mora_ICS"
        ...
        datos = sumar_establecimientos_actividad(
            self.datos,tipo_rpt="excel")
        ws = pintar_encabezado(self, datos, columnas, ws)
        self.fila_num += 1
        ws = tabla_establecimietos_x_actividad_excel(
            self, datos, ws)
