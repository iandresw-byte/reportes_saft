INDU_COMER_SERV_INICIO_TOP_GOB_AVISO ="""
...
SELECT top (@Cantidad_filas) 
    ...
    FC_03.ClaveCatastro, FC_03.CodBarrio, FC_03.CodAldea,
    FC_03.ClaveCatastro as ClavesCatastro,
...
WHERE F_01.AvPgEstado = 1
    ...
    AND F_01.AvPgTipoImpuesto IN (2,3)
    ...
    WHERE A.Identidad = FC_03.DNI
        AND  (A.TipoImpuesto = 2)
        AND  (Estado <> 'Anulado')
        )
GROUP BY  FC_03.DNI, FC_03.Pnombre, FC_03.SNombre, FC_03.PApellido, FC_03.SApellido, FC_03.Direccion, FC_03.ClaveCatastro, FC_03.ClaveCatastro, FC_03.CodBarrio, FC_03.CodAldea
HAVING CAST(SUM(F_02.ValorUnitAvPgDet) AS FLOAT)  > @ValorUnitAvPgDet
ORDER BY  FC_03.DNI
    """

INDU_COMER_SERV_INICIO_TOP_GOB ="""
DECLARE @Cantidad_filas INT = ?
DECLARE @FechaVenceAvPg DATE = ?
DECLARE @AvPgTipoImpuesto VARCHAR(1) =?
DECLARE @Tipo VARCHAR(1) =?
...
ORDER BY  FC_03.DNI
    """
INDU_COMER_SERV_INICIO_TOP_SAMI ="""
DECLARE @Cantidad_filas INT = ?
DECLARE @FechaVenceAvPg DATE = ?
DECLARE @AvPgTipoImpuesto VARCHAR(1) =?
DECLARE @Tipo VARCHAR(1) =?
