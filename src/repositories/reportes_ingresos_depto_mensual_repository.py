...
class IngresosDeptosMensualRepository:
    ...
    def obtener_mensual_ics_otras(self, anio: str):
        query = """month(F_03.FechaRecibo) as Mes,
                    ...
                    F_01 ON Usuario.UsuarioCod = F_01.CreadoPor ON F_04.NumFactura = F_01.NumAvPg INNER JOIN
                    FC_03 ON F_03.DNI = FC_03.DNI
                    WHERE        (F_03.ReciboAnulado = 0) AND (YEAR(F_03.FechaRecibo) = ?) AND (PLA_DEPARTAMENTOS.IdDeptos = 2) AND (F_01.AvPgTipoImpuesto in (0))
                    GROUP BY month(F_03.FechaRecibo)
                    ORDER BY Mes
                    """
        with self.conexion.cursor() as cur:
    ...
    def obtener_mensual_ics_bienes(self, anio: str):
        query = """SELECT month(F_03.FechaRecibo) as Mes, 
                    ...
                    F_01 ON Usuario.UsuarioCod = F_01.CreadoPor ON F_04.NumFactura = F_01.NumAvPg INNER JOIN
                    FC_03 ON F_03.DNI = FC_03.DNI
                    WHERE        (F_03.ReciboAnulado = 0) AND (YEAR(F_03.FechaRecibo) = ?) AND (PLA_DEPARTAMENTOS.IdDeptos = 2) and f_01.AvPgTipoImpuesto in (1,8)
    ...
    def obtener_mensual_ics_comercio(self, anio: str):
        query = """
                ...
                WHERE        (F_03.ReciboAnulado = 0) AND (YEAR(F_03.FechaRecibo) = ?) AND (PLA_DEPARTAMENTOS.IdDeptos = 2) AND (F_01.AvPgTipoImpuesto in (2,3,8))
    ...
    def obtener_mensual_ics_personal(self, anio: str):
        query = """
                ...
                F_01 ON Usuario.UsuarioCod = F_01.CreadoPor ON F_04.NumFactura = F_01.NumAvPg INNER JOIN
                FC_03 ON F_03.DNI = FC_03.DNI
                WHERE        (F_03.ReciboAnulado = 0) AND (YEAR(F_03.FechaRecibo) = ?) AND (PLA_DEPARTAMENTOS.IdDeptos = 2) AND (F_01.AvPgTipoImpuesto in (4,8))
