...
from src.ui.components.ui_colors import color_borde, color_bg, color_texto, color_icon_loguin
...
def txt_label_text_tabs(text=''):
    return Text(text, size=12.5, color=borde_color, width=500,
                col=12, weight=FontWeight.W_400, text_align=TextAlign.START,)


def create_texFiel_fijas(label_text="", value='', visible=True, read_only=True, width=None, ref=None, on_change=None, disabled=False, color=text_color) -> TextField:
    return TextField(
        label=label_text,
        value=value,
        expand=True,
        ...
        label_style=TextStyle(size=text_sise, font_family="Tahoma",
                              color=color, weight=FontWeight.W_400,),
        text_style=TextStyle(size=text_sise, font_family="Tahoma", color=color, weight=FontWeight.W_400,),)
...
def create_text_table(valor: str, color=text_color) -> Text:
    return Text(
        value=valor,
        color=color,
        font_family="Tahoma",
...
def create_text_paginacion(valor: str, color=text_color) -> Text:
    return Text(
        value=valor,
        color=color,
        font_family="Tahoma",
...
def create_text_data_general_titulo(texto: str) -> Text:
    return Text(texto, size=18, weight=FontWeight.BOLD,
                style=TextStyle(color=text_color, font_family="Tahoma"))


def create_text_data_general_labes(texto: str) -> Text:
    return Text(texto, style=TextStyle(
        color=text_color, font_family="Tahoma"))
