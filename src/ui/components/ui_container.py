from flet import (Container, Animation, AnimationCurve, Offset,   alignment,  # type: ignore
                  Column, BoxShadow, Text, Row, CrossAxisAlignment, MainAxisAlignment, ScrollMode, ShadowBlurStyle)

from src.ui.components.ui_colors import color_bg, container_color,  color_bg_2, color_texto
...
def create_container_tabla(tabla) -> Container:

    return Container(
        content=Row(
            controls=[tabla],   # tu DataTable aquí
...
def create_container_card(card) -> Container:
    return Container(
        padding=15,
        border_radius=10,
        bgcolor=color_bg_2(),
