...
def abrir_modal_rpt_bomberos(vista, e):
    fecha = date.today()
    anio = fecha.year
    mes = fecha.month
    ...
    btn_excel.on_click = vista.generar_excel_ingreso_bomberos
    btn_pdf = create_boton_pdf()
    btn_pdf.on_click = vista.generar_pdf_ingreso_bomberos
    btn_salir = create_boton_salir_modal()

    btn_salir.on_click = lambda _: vista.cerrar_modal()
    txt_titulo = create_titulo_modal(
        "Ingresos Cuenta de Servicio de Bomberos")
